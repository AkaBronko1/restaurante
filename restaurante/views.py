...
def index_user(request):
    ...
    platillos_mas_vendidos = Orden.objects.filter(estatus='pagada').values('detalles__platillo__nombre').annotate(total_vendido=Sum('detalles__cantidad')).order_by('-total_vendido')[:10]
